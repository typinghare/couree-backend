...
public class RequestExceptionManager {
    ...
    @Autowired
    public RequestExceptionManager(ErrorConfiguration errorConfiguration, ApplicationContext applicationContext) {
    ...
    public String getErrorCode(RequestException requestException) {
        final RequestExceptionClass requestExceptionClass = getRequestExceptionClass(requestException);
        final int entryCode = requestExceptionClass.getRequestExceptionEntry(requestException).entryCode;
        final int requestExceptionCodeLength = errorConfiguration.requestExceptionCodeLength.intValue();

        return requestExceptionClass.requestExceptionCode +
            Strings.padStart(String.valueOf(entryCode), requestExceptionCodeLength, '0');
    }
